...
class Executor:
    ...
    def _web_fetch(self, url: str, parse: bool = True) -> str:
        """Fetch the content of a URL. If parse is True, returns the text content."""
        self.actions.append(f"web_fetch {url}")
        try:
            response = requests.get(url, timeout=10, headers={"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36"})
